...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    function updateProgress(percent, current) {
      safeUpdateProgress(percent, current, totalMessages, `Processing messages... ${current || 0} found`);
    }
          ...
          if (
            ...
            if (messageData.type === "message") {
              ...
              if (
                ...
                // ENHANCED duplicate check with better logic
                let isDuplicate = false;
                const currentContentLower = messageData.content.toLowerCase().trim();
                
                // Create content hash for all conversations
                ...
                if (window.contentHashSet.has(contentHash)) {
                  ...
                  for (const [key, existingMsg] of messageMap.entries()) {
                    ...
                    if (existingMsg.content === messageData.content && 
                      ...
                      isDuplicate = true;
                      console.log(`🔄 Exact duplicate found in Map: "${messageData.content.substring(0, 50)}..."`);
                      break;
                    }
                  }
                  
                  // STEP 3: For smaller conversations, also check similarity
                  if (!isDuplicate && messageMap.size <= 5000) {
                    // Check similarity only against recent messages for performance
                    ...
                    for (const existingMsg of recentMessages) {
                      if (existingMsg.sender === messageData.sender) {
                        ...
                        if (similarity > 0.95) { // Increased threshold to reduce false positives
                  ...
                  // STEP 4: Only add to hash set if NOT a duplicate
                  if (!isDuplicate) {
    ...
    async function performAdvancedScroll() {
      ...
      try {
        ...
        console.log("Could not register message listener (extension context issue):", error.message);
      }
      ...
      while (scrollAttempts < dynamicMaxRetries && !userRequestedStop) {
        ...
        // Check if extension context is still valid, but continue anyway
        if (!isExtensionContextValid()) {
          console.log("⚠️ Extension context invalidated, but continuing with message extraction...");
        ...
        safeUpdateProgress(
          ...
          currentMessageCount,
          currentMessageCount,
          `Loading messages... (${currentMessageCount} found messages, ${scrollAttempts}/${dynamicMaxRetries} attempts${efficiencyInfo})`
        );
        ...
        if (isLazyLoading) {
          ...
          safeUpdateProgress(
            Math.min(80, (scrollAttempts / maxRetries) * 80),
            currentMessageCount,
            currentMessageCount,
        ...
        if (newMessageCount > currentMessageCount) {
          ...
          if (currentMessageCount > 5000) {
            ...
            safeUpdateProgress(
              Math.min(80, (scrollAttempts / dynamicMaxRetries) * 80),
              currentMessageCount,
              currentMessageCount,
              `Loading massive conversation... (${currentMessageCount} messages, ETA: ${etaMinutes}min, ${messagesPerAttemptCurrent.toFixed(1)} msg/attempt)`
            );
          } else if (currentMessageCount > 2000) {
            safeUpdateProgress(
              Math.min(80, (scrollAttempts / dynamicMaxRetries) * 80),
              currentMessageCount,
              currentMessageCount,
              `Loading large conversation... (${currentMessageCount} messages, ${scrollAttempts}/${dynamicMaxRetries} attempts)`
            );
            ...
            safeUpdateProgress(
              ...
              currentMessageCount,
              currentMessageCount,
              `Loading conversation... (${currentMessageCount} messages, ${scrollAttempts} attempts)`
            );
          }
    ...
    async function extractAllMessages() {
      try {
        ...
        updateProgress(5, 0);
        safeUpdateProgress(10, 0, 0, "Starting dynamic scrolling to load all messages...");
        ...
        safeUpdateProgress(85, messageMap.size, messageMap.size, `Processing ${messageMap.size} messages from Map...`);
        ...
        if (allMessages.length > 0) {
          ...
          try {
            ...
            console.log("Failed to send no messages error:", error.message);
          }
          console.log("No messages found. Make sure you are viewing a conversation with messages.");
          return;
        }
        ...
        try {
          ...
          console.log("Failed to send error message:", sendError.message);
        }
        console.error("Export error:", error);
      }
    }
